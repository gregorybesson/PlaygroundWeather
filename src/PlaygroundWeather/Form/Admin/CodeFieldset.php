...
class CodeFieldset extends Fieldset implements InputFilterProviderInterface
{
    ...
    public function __construct($name = null, ServiceManager $serviceManager, Translator $translator)
    {
        ...
        $this->setHydrator(new DoctrineHydrator($entityManager, 'PlaygroundWeather\Entity\Code'))
        ->setObject(new \PlaygroundWeather\Entity\Code());
    ...
    {
        $codes = array();
        $codeService = $this->getServiceManager()->get('playgroundweather_code_service');
        $results = $codeService->getCodeMapper()->findBy(array(
            'isDefault' => 0
        ));

        foreach ($results as $result) {
            $codes[$result->getId()] = $result->getValue() . ' - ' . $result->getDescription();
        }
